...
test.describe('safety.viz shift-plot module', () => {
  ...
  test('SSP-API-003/PPRF-SSP-004: brushing dispatches participantsSelected on the shell root, bubbling to the element (#14, #99)', async ({
    ...
    const events = await page.evaluate(() => {
      const instance = window.__safetyShiftPlotInstance;
      // The dispatch target moved to the shell root in the rail adoption
      // (#99, PPRF-SSP-004) so root-level listeners — the railed profile's
  ...
  test('PPRF-SSP-001: a multi-participant brush collapses the rail to a worst-first stepper whose steps emphasize the chart (#99)', async ({
    page
  }) => {
    await page.evaluate(() => window.__safetyShiftPlotInstance.brushValues(9.5, 13.5, 9.5, 16));
    // Worst-first: S03 has the highest peak ×ULN (15/20) of the brushed four.
    await expect(page.locator('.sv-rail .sv-profile-step-count')).toContainText('1 of 4');
    await expect(page.locator('.sv-rail .sv-profile-id')).toHaveText('Participant S03');
    ...
    await captureEvidence(page, 'PPRF-SSP-001', 'railed-stepper');
    ...
    await expect(page.locator('.sv-rail .sv-profile-step-count')).toContainText('2 of 4');
  ...
  test('PPRF-SSP-002: a single-point brush shows the full railed profile with no stepper, beside the linked listing (#99)', async ({
    page
  }) => {
    ...
    await expect(page.locator('.sv-rail .sv-profile-id')).toHaveText('Participant S01');
    ...
    await expect(page.locator('.sv-rail .sv-profile-measure-row')).toHaveCount(1);
    // The linked listing stays — records vs story (PPRF-11).
    await expect(page.locator('.sv-listing table')).toBeVisible();
    await captureEvidence(page, 'PPRF-SSP-002', 'railed-full-profile');
  });

  test('PPRF-SSP-003: clearing the selection and control-driven redraws empty the rail (#99)', async ({
    page
  }) => {
    await page.evaluate(() => window.__safetyShiftPlotInstance.brushValues(9.5, 13.5, 9.5, 16));
    await expect(page.locator('.sv-rail .sv-profile-step-count')).toContainText('1 of 4');
    ...
    // Re-brush, then clear through the rail's own Clear affordance: it routes
    ...
    // A control-driven render resets the selection silently — the rail must
