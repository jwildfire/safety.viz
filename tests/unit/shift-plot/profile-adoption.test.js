...
// selection path — showSelection → dispatchSelected — feeds the railed module,
...
// catches several participants, so the rail collapses to the worst-first
...
describe('shift-plot participant-profile adoption (PPRF-SSP-002)', () => {
  it('PPRF-SSP-002: mounts the rail by default (config-on) into the shell profile slot', () => {
  ...
  it('PPRF-SSP-002: derives the rail feed rows ONCE per setData from rawData through the hep-core cleaners', () => {
    const rows = makeRows();
  ...
  it('PPRF-SSP-002: host listing columns are NOT the rail header details (profile_details defaults empty)', () => {
    const instance = build();
    // The host `details` default to the pair columns (baseline/comparison/
    // change) — per-pair fields, not demographics — so the rail header shows
    // none.
...
describe('shift-plot cohort stepper (PPRF-SSP-001, PPRF-11)', () => {
  it('PPRF-SSP-001: a multi-participant brush collapses the rail to a worst-first stepper', () => {
    const instance = build();
  ...
  it('PPRF-SSP-001: stepping emphasizes the stepped point on the chart without re-dispatching', () => {
    ...
    expect(instance.railWrap.querySelector('.sv-profile-id').textContent).toBe('Participant P3');
...
describe('shift-plot rail clear paths (PPRF-SSP-003, PPRF-11)', () => {
  it('PPRF-SSP-003: clearing the selection (the tiny-click path) empties the rail and the emphasis', () => {
    const instance = build();
    brush(instance, ['P3', 'P1']);
    instance.railWrap.querySelector('.sv-profile-step-next').click();
    instance.clearSelection();
  ...
  it('PPRF-SSP-003: the rail Clear affordance routes through the host clear path', () => {
    const instance = build();
    const heard = [];
    instance.root.addEventListener('participantsSelected', (event) =>
      heard.push(event.detail.data)
  ...
  it('PPRF-SSP-003: dock Clear empties an externally-fed cohort the host never selected (PPRF-11)', () => {
    ...
    // shape, #87): no brush, no $sspSelected, but the rail fills.
  ...
  it('PPRF-SSP-003: a control-driven render resets the selection AND the rail (render preamble)', () => {
    const instance = build();
    brush(instance, ['P1']);
    expect(instance.railWrap.querySelector('.sv-profile-root')).not.toBeNull();
  ...
  it('PPRF-SSP-003: destroy tears the rail down with the instance', () => {
    const instance = build();
    brush(instance, ['P1']);
    instance.destroy();
