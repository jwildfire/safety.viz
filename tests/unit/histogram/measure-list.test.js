...
// The `measures` setting on histogram (SH-MEAS-001/002, #136): an ordered
...
const { default: factory } =
  await import('/Users/jwildfire/Documents/obot2/safety.viz/.claude/worktrees/legacy33/src/histogram.js');
...
const measureOptions = (instance) => {
  ...
  return [...control.querySelectorAll('option')]
    ...
    .filter((text) => text !== 'All Measures');
};

describe('histogram: the measures whitelist', () => {
  it('SH-MEAS-001: no whitelist offers every measure in the data, alphabetically (#136)', () => {
  ...
  it('SH-MEAS-001: a whitelist offers exactly the listed measures, in the listed order (#136)', () => {
  ...
  it('SH-MEAS-002: a configured measure absent from the data is dropped with a warning (#136)', () => {
  ...
  it('SH-MEAS-002: a whitelist matching nothing falls back to every measure (#136)', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
  ...
  it('SH-MEAS-001: setSettings re-resolves the control against the new whitelist (#136)', () => {
