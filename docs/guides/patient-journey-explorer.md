...
Every other renderer in this gallery looks across participants. This one looks at **one participant** and shows their whole safety record on a single study-day axis, so the question "what else was going on when this happened?" can be answered by looking rather than by joining six listings by hand. Pick a participant and the page draws stacked lanes on one shared timeline:
...
4. **Prior adverse events with the same preferred term** — every earlier event for this participant with the anchor's preferred term (falling back to the verbatim term when the preferred term is blank), most recent first. This list is deliberately not limited to the window: it answers "has this happened to them before?", and the answer is not confined to the last thirty days.
...
That last case is the one to read carefully, because in this study it is nearly every con-med: the con-med data carries no ongoing indicator at all and 91% of its records have no end date. The module counts those records as active, because excluding them would print "0 con-meds active" and assert that nothing was co-administered, which the data does not support either. It also names the count: when every listed con-med has an unrecorded end the panel says, directly under the heading, "None of these 7 has a recorded end date; they are shown as active because nothing records them stopping." When only some do, it says how many. Read the number and the sentence together.

A record whose end day is earlier than its start day is a data error, not an open interval. It is kept, drawn as a single-day mark at its start, and counted in a note above the lanes, so it can be found and fixed rather than quietly redrawn as ongoing.
...
- **81% of con-meds are `UNCODED`**: their ATC class was never assigned. `UNCODED` is kept as its own selectable bucket in the class filter rather than hidden, and the sidebar says how large it is.
...
- **No `HH` / `LL` lab tier** — the indicator column carries only normal, high, low and abnormal — so the escalation glyph and its ring are implemented but never drawn on this page. Onset days for medical history are also sparsely populated (17%), which is why the history lane plots the collection day and says "at screening".

The demo opens on participant `01-716-1447` because they are the one participant whose ±30-day window contains every kind of context at once. Anchor the ERYTHEMA event at day 30 and the panel shows **7 con-meds active** (all 7 with no recorded end date), **2 more starting later in the window** (cortisone and a topical steroid at day 43 — a visible response to the rash), **1 abnormal lab** (the 1.06 × ULN AST at day 27), **1 dose change** (54 → 81 mg at day 17) and **0 prior adverse events** with the same preferred term. The hyperhidrosis at day 17 is a different term, so it is a co-occurring event in the window, not a recurrence. Those five numbers are what the browser suite asserts against the built demo page.
...
- **Medical history is plotted where it was collected**, not where it began, because onset is rarely recorded in this study; the tooltip names the onset when one exists and the relative timing ("before study") when it does not. A host with complete onset dates can switch the lane to onset.
