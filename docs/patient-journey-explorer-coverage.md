...
widget follow-on
[obot.roadmap#350](https://github.com/jwildfire/obot.roadmap/issues/350); AI
narrative layer under
[#146](https://github.com/jwildfire/safety.viz/issues/146), requirement
[obot.roadmap#351](https://github.com/jwildfire/obot.roadmap/issues/351)), per
...
grammar admits no digits), `PJE-DEMO-*` (the vendored CDISC Pilot 01
extracts, the built demo page, and the Definition of Done executed against it),
and `PJE-NARR-*` (the AI narrative layer added under
[#146](https://github.com/jwildfire/safety.viz/issues/146), requirement
[obot.roadmap#351](https://github.com/jwildfire/obot.roadmap/issues/351): the
compiled skills folder, the six read-only tools, the generation runtime with its
grounding hash and tool-call cap, the output validator and the refusal cases,
provenance, the provider adapters, the cards with their draft chips, citations,
staleness and reviewer actions, the eval harness and its CI gate, the demo
strip, and the style guide's forbidden-phrase block).
...
| PJE-NARR-009   | PJE-NARR-009       | #146  | the participant-summary card sits above the lanes as a blurb that expands to the full cited draft, and an unbound slot renders nothing (also `narratives-renderer.test.js`, jsdom)                  |
| PJE-NARR-010   | PJE-NARR-010       | #146  | anchoring drafts the event context at the top of the panel body, every sentence prefixed with the draft chip, and a refusal renders the catalog text                                                |
| PJE-NARR-011   | PJE-NARR-011       | #146  | a citation chip lights the cited mark and moves focus to it; a cited row not on the timeline opens its source record                                                                                |
| PJE-NARR-012   | PJE-NARR-012       | #146  | a window change regenerates the event narrative with a regenerate action; a filter that changes the grounding rows greys the card and offers Regenerate (also `narratives-renderer.test.js`, jsdom) |
| PJE-NARR-013   | PJE-NARR-013       | #146  | accept, reject, edit and regenerate emit on all three channels with the draft, and an accepted draft passed back flips the chips (also `narratives-renderer.test.js`, jsdom)                        |
| PJE-NARR-014   | PJE-NARR-014       | #146  | lab, dose and disposition narratives are drafted on request from the tray beneath the lanes and render there                                                                                        |
| PJE-NARR-016   | PJE-NARR-016       | #146  | `site.spec.js` — the built demo page (with its AI narratives strip) mounts with no console error; no dedicated row test yet, see the gaps below                                                     |
...
| PJE-NARR-001   | PJE-NARR-001       | #146  | `narratives-skills.test.js` — every skill's front matter, grounding tool, declared tools and Input/Draft/Output definitions; the committed `skills.generated.js` against a fresh compile of the folder             |
| PJE-NARR-002   | PJE-NARR-002       | #146  | `narratives-tools.test.js` — each of the six tools' rows and their `row_id`s; `get_context_window` against `buildContext` row for row; every failure returned as an `{ error }` object                             |
| PJE-NARR-003   | PJE-NARR-003       | #146  | `narratives-runtime.test.js` — the grounding tool runs first, `input_hash` is the canonical-JSON SHA-256 over skill, version, inputs and grounding rows, and the cache key carries skill@version, hash and adapter |
| PJE-NARR-004   | PJE-NARR-004       | #146  | `narratives-runtime.test.js` — an undeclared tool is refused to the model, the tool-call cap holds, and every row a tool returns joins the citation scope                                                          |
| PJE-NARR-005   | PJE-NARR-005       | #146  | `narratives-validator.test.js` — the Draft schema, out-of-scope citations dropped and recorded, forbidden constructs as hard failures, the sentence cap, `status: 'draft'` always                                  |
| PJE-NARR-006   | PJE-NARR-006       | #146  | `narratives-runtime.test.js` — one retry with the errors fed back then `refused:validation`; empty grounding refuses with no model call; provider error, provider refusal and cancellation map to their flags      |
| PJE-NARR-007   | PJE-NARR-007       | #146  | `narratives-runtime.test.js` — provenance completeness on drafts and on refusals: model, skill@version, input hash, timestamp, tool calls grounding-first, adapter, usage, attempts, dropped                       |
| PJE-NARR-008   | PJE-NARR-008       | #146  | `narratives-adapters.test.js` — one `messages()` interface across the three adapters; Claude over raw HTTP sending effort and not temperature; the OpenAI translation both ways; the stub's citations all resolve  |
| PJE-NARR-015   | PJE-NARR-015       | #146  | `narratives-evals.test.js` — the golden set's size and spread across the five skills, the rule-based graders, the per-skill thresholds and the report the run writes                                               |
| PJE-NARR-017   | PJE-NARR-017       | #146  | `narratives-style.test.js` — the forbidden block's reject list and the hedging vocabulary it must accept; `sha256Hex` against known digests                                                                        |
...
- **Out of scope by design**, each needing its own decision: cohort similarity,
  cross-study aggregation, a QT/ECG lane, a dark-mode toggle (the tokens ship,
  the switch does not), a "zoom to window" control, and the R widget binding
  ([obot.roadmap#350](https://github.com/jwildfire/obot.roadmap/issues/350)).
  LLM narratives were on this list and came off it: they landed under
  [#146](https://github.com/jwildfire/safety.viz/issues/146) (requirement
  [obot.roadmap#351](https://github.com/jwildfire/obot.roadmap/issues/351)) as
  the `PJE-NARR-*` rows above.
- **PJE-NARR-016 has no dedicated row test.** The demo page's AI narratives
  strip is exercised only by the existing demo-page tests in
  `tests/e2e/site.spec.js` — the per-renderer shell smoke test, which mounts the
  built page and fails on a console error, and PJE-DEMO-003. A row test
  asserting that the page binds the stub adapter by default and that the
  provider toggle reveals the key field is a follow-up.
- **The eval gate runs against the stub adapter only.** `npm run eval:narratives`
  in CI uses the deterministic offline adapter under the rules grader, because
  a gate must not need a key or a network. Scores against a real provider, and
  the LLM-as-judge second opinion, are run by hand and never gate a merge, so
  the CI signal is that the harness, the tools and the validator hold — not that
  a given model's prose does.
